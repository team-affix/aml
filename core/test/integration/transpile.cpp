// Integration tests: aml_expr (built directly) → lc_expr via lc_transpiler_manifest.
//
// Each test constructs an aml_expr using aml_expr_pool, then feeds it through
// a lc_transpiler_manifest (real lc_expr_pool + scope + all sub-transpilers wired
...
// lc_transpiler_manifest end-to-end behaviour with concrete inputs.
...
struct TranspileIntegrationTest : public ::testing::Test {
    ...
    const lc_expr* transpile(lc_transpiler_manifest& b, const aml_expr* e,
    ...
    const lc_expr* transpile_builtins(lc_transpiler_manifest& b, const aml_expr* e) {
...
// ---------------------------------------------------------------------------
// 3-level nested abstraction through a real lc_transpiler_manifest
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Two lc_transpiler_manifest instances are fully independent
// ---------------------------------------------------------------------------

TEST_F(TranspileIntegrationTest, TwoManifestInstancesAreIndependent) {
    lc_transpiler_manifest m1, m2;
    m1.sc.push("x");
