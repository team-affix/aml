...
TEST(ProgramAssemblyIntegrationTest, DeclarationGroupThreeConstructors) {
    ...
    EXPECT_EQ(terms[0], lc.make_abs(lc.make_abs(lc.make_abs(lc.make_var(2)))));
    // def (k=1, a=0, n=3): abs(abs(abs(var(1))))
    EXPECT_EQ(terms[1], lc.make_abs(lc.make_abs(lc.make_abs(lc.make_var(1)))));
...
TEST(ProgramAssemblyIntegrationTest, BooleanDeclarations) {
    ...
    EXPECT_EQ(terms[0], lc.make_abs(lc.make_abs(lc.make_var(1))));
    // false (k=1, a=0, n=2): abs(abs(var(0)))
    EXPECT_EQ(terms[1], lc.make_abs(lc.make_abs(lc.make_var(0))));
}
